...
<div class="contenido">
    ...
    <?php
    $res = mysqli_query($conn, "SELECT count(*) FROM  usuario inner join objetoaprendizaje on usuario.Matricula=objetoaprendizaje.MatriculaUsuario where Email='$mail'") or die("Error en:" . mysql_error());
    $fi = mysqli_fetch_array($res, MYSQLI_NUM);
    $num = $fi[0];
    ?>
    <!--<button class="btn btn-success registro-nuevo" type="button" name="button"
        <?php if ($num < 20) { ?>
            onclick="swap();"
        <?php } else { ?>onclick="alert('No puede subir mas archivos');" <?php } ?>
    >Nuevo Objeto</button>-->
    ...
    <div class="fixed-action-btn">
        <a name="button"
            <?php if ($num < 20) { ?>
                onclick="swap();"
            <?php } else { ?>onclick="alert('No puede subir mas archivos');" <?php } ?>
           class="btn-floating btn-large tooltipped registro-nuevo" data-position="left"
           data-delay="50" data-tooltip="Nuevo Objeto">
                    ...
                    <table id="listaD" class="table table-bordered text-center table-striped tablesorter" border="1">
                        ...
                        for ($i = 0; $i < $result->num_rows; $i++) {
                            ...
                            <tr>
                                ...
                                <td style='display:none;' class="hidden"><?php echo $extraido[6] ?></td>
                                <td>
        ...
        <form enctype="multipart/form-data" id="formulario" action="Accion/SubirArchivo.php" method="post" role="form"
            ...
            <div class="container">
                <div class="panel panel-body">
                    ...
                    <div class="col-xs-12 col-sm-4">
                        <div class="form-group">
                            <label for="tipo" name="archivo" id="archivo">Seleccionar Archivo</label>
                            <input enctype="multipart/form-data" type="file" name="file" id="file" accept=".zip"/>
                        </div>
                    </div><br>
                ...
                <div class="botones-accion">
                    ...
                    <a href="MiObjeto.php" class="btn btn-primary">Regresar a la Lista</a>
